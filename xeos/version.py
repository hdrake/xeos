"""xeos: version information.

The version is derived from the git tag at build time by ``hatch-vcs``, which
writes the resolved value to ``xeos/_version.py``. That file is generated rather
...
install -- so the fallback below only fires when ``xeos`` is imported straight
from a source checkout that has never been built or installed.
"""

try:
    from xeos._version import __version__
