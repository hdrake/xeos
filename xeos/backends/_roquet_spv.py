"""Roquet et al. (2015) 55-term polynomial for *specific volume* (TEOS-10).
...
standard." Ocean Modelling, 90, 29-43.

The specific-volume form; MOM6 ``EQN_OF_STATE='ROQUET_SPV'``. Coefficients ported
from Fabien Roquet's reference ``polyTEOS10_55t`` / MOM6 ``MOM_EOS_Roquet_SpV.F90``.

Note on the salinity offset: the reduced salinity uses ``deltaS = 24`` (MOM6
...
_CTu = 40.0
_Zu = 1.0e4
_dS = 24.0  # NOT 32 — see module docstring

# Vertical reference profile of specific volume v0(pp).
_V0 = (-4.4015007269e-05, 6.9232335784e-06, -7.5004675975e-07,
       1.7009109288e-08, -1.6884162004e-08, 1.9613503930e-09)

# Specific-volume anomaly coefficients V_{ijk}.
...
# Thermal-expansion (alpha numerator) coefficients.
...
# Haline-contraction (beta numerator) coefficients.
...
def _specific_volume(ss, tt, pp):
    V00, V01, V02, V03, V04, V05 = _V0
...
def _alpha_num(ss, tt, pp):
    ...
    return ((ap3 * pp + ap2) * pp + ap1) * pp + ap0


def _beta_num(ss, tt, pp):
    bp3 = B003
    ...
    return ((bp3 * pp + bp2) * pp + bp1) * pp + bp0


def density(ct, sa, p_dbar):
    ss, tt, pp = _reduced(ct, sa, p_dbar)
    return 1.0 / _specific_volume(ss, tt, pp)


def specific_volume(ct, sa, p_dbar):
    ss, tt, pp = _reduced(ct, sa, p_dbar)
    return _specific_volume(ss, tt, pp)


def drho_dt(ct, sa, p_dbar):
    # alpha = a/specvol = -(1/rho) drho/dCT  ->  drho/dCT = -alpha * rho
    ss, tt, pp = _reduced(ct, sa, p_dbar)
    spv = _specific_volume(ss, tt, pp)
    alpha = _alpha_num(ss, tt, pp) / spv
    return -alpha / spv  # rho = 1/spv


def drho_ds(ct, sa, p_dbar):
    # beta = b/ss/specvol = (1/rho) drho/dSA  ->  drho/dSA = beta * rho
    ss, tt, pp = _reduced(ct, sa, p_dbar)
    spv = _specific_volume(ss, tt, pp)
    beta = _beta_num(ss, tt, pp) / ss / spv
    return beta / spv
